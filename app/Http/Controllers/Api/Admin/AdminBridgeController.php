...
class AdminBridgeController extends Controller
{
    ...
    public function productsImport(Request $request): JsonResponse
    {
        $this->ensureAdmin($request);
        ...
        if (!in_array($ext, ['csv', 'txt'])) {
            return response()->json(['success' => false, 'message' => 'Only CSV import is supported via API'], 400);
        }
        ...
        if (!$headers) {
            fclose($handle);
            return response()->json(['success' => false, 'message' => 'Empty or invalid CSV file'], 400);
        }
        ...
        if ($nameIdx === false || $priceIdx === false) {
            fclose($handle);
            return response()->json(['success' => false, 'message' => 'CSV must have at least "name" and "price" columns'], 400);
        }
        ...
        while (($row = fgetcsv($handle)) !== false) {
            ...
            if ($name === '' || $price <= 0) {
                $errors[] = "Row skipped: empty name or invalid price";
                continue;
            }

            try {
                DB::transaction(function () use ($row, $name, $price, $skuIdx, $stockIdx, $categoryIdx, $request, &$imported) {
                    $product = new Product();
                    $product->name = $name;
                    $product->slug = $this->uniqueSlug(Str::slug($name));
                    $product->unit_price = $price;
                    $product->purchase_price = $price;
                    $product->added_by = 'admin';
                    $product->user_id = $request->user()->id;
                    $product->published = 1;
                    $product->current_stock = $stockIdx !== false ? (int) ($row[$stockIdx] ?? 0) : 0;
                    $product->save();

                    $product->stocks()->create([
                        'variant' => '',
                        'sku' => $skuIdx !== false ? ($row[$skuIdx] ?? null) : null,
                        'price' => $price,
