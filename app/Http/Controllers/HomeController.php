...
class HomeController extends Controller
{
    ...
    public function login()
    {
        ...
        $layout = get_setting('authentication_layout_select') ?: 'free';

        if (Route::currentRouteName() == 'admin.login') {
            return view('auth.' . $layout . '.admin_login');
        } else if (Route::currentRouteName() == 'seller.login' && get_setting('vendor_system_activation') == 1) {
            return view('auth.' . $layout . '.seller_login');
        } else if (Route::currentRouteName() == 'deliveryboy.login' && addon_is_activated('delivery_boy')) {
            return view('auth.' . $layout . '.deliveryboy_login');
        }
        return view('auth.' . $layout . '.user_login');
    }
