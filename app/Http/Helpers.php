...
if (!function_exists('storefront_url')) {
    function storefront_url(?string $path = null): string
    {
        $base = (string) (config('app.frontend_url') ?: config('app.url') ?: url('/'));
        $base = rtrim($base, '/');
...
if (!function_exists('cart_product_tax')) {
    function cart_product_tax($cart_product, $product, $formatted = true)
    {
        ...
        if ($product->gst_rate > 0) {
            $tax += ($price * $product->gst_rate) / 100;
        }
...
function getShippingCost($carts, $index, $shipping_info = '', $carrier = '')
{
    ...
    if ($shipping_type == 'flat_rate') {
        ...
        // Zone lookup: city → state → zone_id (falls back to country → zone_id)
        $user_zone = 0;
        if (!empty($shipping_info['city_id'])) {
            $shippingCity = City::where('id', $shipping_info['city_id'])->first();
            if ($shippingCity && $shippingCity->state_id) {
                $shippingState = State::where('id', $shippingCity->state_id)->first();
                if ($shippingState && $shippingState->zone_id) {
                    $user_zone = $shippingState->zone_id;
                }
            }
        }
        if ($user_zone == 0 && !empty($shipping_info['state_id'])) {
        ...
        if ($user_zone == 0 && !empty($shipping_info['country_id']) && $shipping_info['country_id'] != 0) {
            $shippingCountry = Country::where('id', $shipping_info['country_id'])->first();
            $user_zone = $shippingCountry ? ($shippingCountry->zone_id ?? 0) : 0;
        }
        ...
        foreach ($carrier->carrier_ranges as $carrier_range) {
            if ($itemsWeightOrPrice >= $carrier_range->delimiter1 && $itemsWeightOrPrice < $carrier_range->delimiter2) {
                $rangePrice = $carrier_range->carrier_range_prices->where('zone_id', $user_zone)->first();
...
if (!function_exists('seller_base_carrier_list')) {
    function seller_base_carrier_list($owner_id, $userId = null, $tempUserId= null, $shipping_info = null)
    {
        ...
        if (count($carts) > 0) {
            ...
            $zone = null;
            if (!empty($shipping_info['city_id'])) {
                $szCity = City::where('id', $shipping_info['city_id'])->first();
                if ($szCity && $szCity->state_id) {
                    $szState = State::where('id', $szCity->state_id)->first();
                    if ($szState && $szState->zone_id) {
                        $zone = $szState->zone_id;
                    }
                }
            }
            if (!$zone && !empty($shipping_info['country_id'])) {
                $szCountry = Country::where('id', $shipping_info['country_id'])->first();
                $zone = $szCountry ? ($szCountry->zone_id ?: null) : null;
            }
...
if (!function_exists('normalize_uploaded_asset_path')) {
    function normalize_uploaded_asset_path($path)
    {
...
if (!function_exists('is_direct_asset_reference')) {
    function is_direct_asset_reference($reference)
    {
        ...
        $normalized = normalize_uploaded_asset_path($reference);
...
if (!function_exists('local_public_asset_path')) {
    function local_public_asset_path($path)
    {
        $normalized = normalize_uploaded_asset_path($path);
...
if (!function_exists('uploaded_asset')) {
    function uploaded_asset($id)
    {
        ...
        if (is_direct_asset_reference($id) && !ctype_digit((string) $id)) {
            $path = normalize_uploaded_asset_path($id);

            return $path ? my_asset($path) : static_asset('assets/img/placeholder.jpg');
        }
...
if (!function_exists('uploaded_asset_path')) {
    ...
    {
        $placeholderPath = local_public_asset_path('assets/img/placeholder.jpg');

        if (blank($id)) {
            return $placeholderPath ?: static_asset('assets/img/placeholder.jpg');
        }
        ...
        if (is_direct_asset_reference($id) && !ctype_digit((string) $id)) {
            $localPath = local_public_asset_path($id);

            if (config('filesystems.default') === 'local' && $localPath && file_exists($localPath)) {
                return $localPath;
            }
        ...
        if (($asset = Upload::find($id)) != null) {
            ...
            $localPath = local_public_asset_path($asset->file_name);
...
if (!function_exists('build_upload_preview_files')) {
    function build_upload_preview_files($references)
    {
        $references = collect(is_array($references) ? $references : explode(',', (string) $references))
            ->map(fn ($reference) => trim((string) $reference))
            ->filter()
            ->values();

        $numericIds = $references
            ->filter(fn ($reference) => ctype_digit($reference))
            ->values();

        $uploadsById = $numericIds->isNotEmpty()
            ? Upload::query()
                ->whereIn('id', $numericIds->all())
                ->get()
                ->keyBy(fn ($upload) => (string) $upload->id)
            : collect();

        return $references->map(function ($reference) use ($uploadsById) {
            if (ctype_digit($reference)) {
                $file = $uploadsById->get($reference);
            ...
            if (!is_direct_asset_reference($reference)) {
                return null;
            }
...
if (!function_exists('my_asset')) {
    ...
    function my_asset($path, $secure = null)
    {
        ...
        $prefix = request()->server('DOCUMENT_ROOT') === base_path('public') ? '' : 'public/';
        return app('url')->asset($prefix . $path, $secure);
    }
}
...
if (!function_exists('checkout_done')) {
    function checkout_done($combined_order_id, $payment)
    {
        ...
        if ($combined_order->user_id != null) {
            \App\Models\Cart::where('user_id', $combined_order->user_id)->active()->delete();
        }
    }
}
