...
class OrderController extends Controller
{
    ...
    {
        $paymentType = AllowedPaymentMethods::normalize($request->payment_type);
        if (! AllowedPaymentMethods::isAllowed($paymentType)) {
            return response()->json([
                'combined_order_id' => 0,
                'result' => false,
                'message' => 'Only Razorpay and PhonePe payments are available for checkout.',
                'errors' => [
                    'payment_type' => ['Only Razorpay and PhonePe payments are available for checkout.'],
                ],
            ], 422);
        }
