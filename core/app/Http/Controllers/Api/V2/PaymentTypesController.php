...
class PaymentTypesController
{
    ...
    public function getList(Request $request)
    {
        ...
        if ($list == "online" || $list == "both") {
            $online_payment_methods = PaymentMethod::query()
                ->whereIn('name', AllowedPaymentMethods::METHODS)
                ->when(Schema::hasColumn('payment_methods', 'active'), fn ($query) => $query->where('active', 1))
                ->when(Schema::hasColumn('payment_methods', 'status'), fn ($query) => $query->where('status', 1))
                ->get();
