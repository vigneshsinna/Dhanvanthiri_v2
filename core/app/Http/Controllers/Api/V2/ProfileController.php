...
class ProfileController extends Controller
{
    ...
    public function updateImage(Request $request)
    {
        ...
        try {
            if ($request->hasFile('avatar') || $request->hasFile('file')) {
                $uploaded = $this->storeUploadedFileForUser($user, $request->file('avatar') ?: $request->file('file'));

                $user->avatar_original = $uploaded->id;
                $user->save();

                return response()->json([
                    'result' => true,
                    'message' => translate("Image updated"),
                    'path' => uploaded_asset($uploaded->id)
                ]);
            }
    ...
    private function storeUploadedFileForUser(User $user, UploadedFile $file): Upload
    {
        $type = array(
            "jpg" => "image",
            "jpeg" => "image",
        ...
        if (!isset($type[$extension])) {
            throw new \RuntimeException('Only image can be uploaded');
        }
        ...
        if (env('FILESYSTEM_DRIVER') == 's3') {
            Storage::disk('s3')->put($newPath, file_get_contents($newFullPath), ['visibility' => 'public']);
            unlink($newFullPath);
        }

        $upload = new Upload;
        $upload->file_original_name = pathinfo($file->getClientOriginalName(), PATHINFO_FILENAME) ?: $newFileName;
        $upload->extension = $extension;
        $upload->file_name = $newPath;
        $upload->user_id = $user->id;
        $upload->type = $type[$extension];
        $upload->file_size = File::exists($newFullPath) ? File::size($newFullPath) : (int) $file->getSize();
        $upload->save();
