...
class CheckoutController extends Controller
{
    ...
    public function checkout(Request $request)
    {
        ...
        if (! AllowedPaymentMethods::isAllowed($request->payment_option)) {
            flash(translate('Only Razorpay and PhonePe payments are available.'))->warning();
            return redirect()->route('checkout');
        }
        $user = auth()->user();
        ...
        if (!$this->shouldDeferCartCleanupUntilPaymentConfirmation((string) $request->payment_option) && count($carts) > 0) {
    ...
    public function shouldDeferCartCleanupUntilPaymentConfirmation(string $paymentOption): bool
    {
        ...
        return !in_array(class_basename($decorator), [
    ...
    public function checkout_done($combined_order_id, $payment)
    {
        ...
        // Redirect to React storefront confirmation page
        return redirect(storefront_url('checkout/confirmation?combined_order_id=' . $combined_order_id));
    }
