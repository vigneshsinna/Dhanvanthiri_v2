...
class DhanvathiriProductsSeeder extends Seeder
{
    public function run()
    {
        ...
        foreach ($products as $p) {
            ...
            $uploadId = null;
            if (!empty($p['image'])) {
                $imageRelativePath = $this->resolveProductImagePath($p['image']);

                if ($imageRelativePath) {
                    $absolutePath = public_path($imageRelativePath);
                    $fileSize = filesize($absolutePath);
                    $ext = pathinfo($absolutePath, PATHINFO_EXTENSION);

                    $upload = Upload::withTrashed()->firstOrNew(['file_name' => $imageRelativePath]);
                    $upload->deleted_at = null;
                    $upload->file_original_name = pathinfo($p['image'], PATHINFO_FILENAME);
            ...
            if ($uploadId) {
                $productPayload['photos'] = (string) $uploadId;
                $productPayload['thumbnail_img'] = $uploadId;
            }

            if ($existingProduct) {
                DB::table('products')->where('id', $existingProduct->id)->update($productPayload);
                $productId = $existingProduct->id;
            } else {
                $productPayload['slug'] = $p['slug'];
            ...
            $stockPayload = [
                'sku' => $existingStock->sku ?? 'DV-' . strtoupper(substr($p['slug'], 0, 3)) . '-' . $productId,
                'price' => $p['price'],
                'qty' => 500,
                'updated_at' => $now,
            ];
            ...
            if ($uploadId) {
                $stockPayload['image'] = $uploadId;
            }

            if ($existingStock) {
                DB::table('product_stocks')->where('id', $existingStock->id)->update($stockPayload);
            } else {
                $stockPayload['product_id'] = $productId;
                $stockPayload['variant'] = '';
                $stockPayload['created_at'] = $now;
                DB::table('product_stocks')->insert($stockPayload);
            }

            // Category pivot
            DB::table('product_categories')->updateOrInsert(
                ['product_id' => $productId, 'category_id' => $p['category_id']],
                ['product_id' => $productId, 'category_id' => $p['category_id']]
            );
        }

        if ($this->command) {
            $this->command->info('Dhanvathiri products enriched with RICH METADATA from storefront source.');
        } else {
            dump('Dhanvathiri products enriched with RICH METADATA from storefront source.');
        }
    }
    ...
    private function resolveProductImagePath(string $image): ?string
    {
        ...
        $candidates = [
            'uploads/all/' . $image,
            'uploads/all/legacy-storefront/' . $image,
        ];
        ...
        $wanted = strtolower($image);
        foreach (['uploads/all', 'uploads/all/legacy-storefront'] as $directory) {
            $absoluteDirectory = public_path($directory);
            if (!is_dir($absoluteDirectory)) {
                continue;
            }

            foreach (scandir($absoluteDirectory) ?: [] as $file) {
                if (strtolower($file) === $wanted && is_file($absoluteDirectory . DIRECTORY_SEPARATOR . $file)) {
