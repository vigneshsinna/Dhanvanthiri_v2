                                ...
                                <div class="form-group mb-2">
                                    ...
                                    <div class="input-group" data-toggle="aizuploader" data-type="image"
                                        ...
                                        <input type="hidden" name="photos"
                                            value="{{ \App\Models\Product::sanitizeMediaReference($product->photos) }}"
                                            class="selected-files">
                                    </div>
                                    <div class="file-preview box sm">
                                    </div>
                        ...
                        <div class="bg-white p-3 p-sm-2rem">
                            ...
                            <div class="w-100">
                                ...
                                <div class="form-group row gutters-5">
                                    ...
                                    <div class="col-md-8">
                                        @php $product_colors = json_decode($product->colors) ?? []; @endphp
                                        <select class="form-control aiz-selectpicker" data-live-search="true" data-selected-text-format="count" name="colors[]" id="colors" multiple <?php if (count($product_colors) < 1) echo "disabled"; ?>>
                                            @foreach (\App\Models\Color::orderBy('name', 'asc')->get() as $key => $color)
                                            <option
                                                value="{{ $color->code }}"
                                                data-content="<span><span class='size-15px d-inline-block mr-2 rounded border' style='background:{{ $color->code }}'></span><span>{{ $color->name }}</span></span>"
                                                <?php if (in_array($color->code, $product_colors)) echo 'selected' ?>
                                                ></option>
                                            @endforeach
                                        </select>
                                    </div>
                                    <div class="col-md-1">
                                        <label class="aiz-switch aiz-switch-success mb-0">
                                            <input value="1" type="checkbox" name="colors_active" <?php if (count($product_colors) > 0) echo "checked"; ?> >
                                            <span></span>
                                        </label>
                                    </div>
                                </div>
